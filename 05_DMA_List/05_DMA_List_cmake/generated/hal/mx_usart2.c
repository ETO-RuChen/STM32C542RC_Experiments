...
static hal_dma_handle_t hLPDMA2_CH0;
...
hal_uart_handle_t *mx_usart2_uart_init(void)
{
  ...
  /* Enable interrupt */
  HAL_CORTEX_NVIC_SetPriority(USART2_IRQn, HAL_CORTEX_NVIC_PREEMP_PRIORITY_2, HAL_CORTEX_NVIC_SUB_PRIORITY_0);
  HAL_CORTEX_NVIC_EnableIRQ(USART2_IRQn);

  /* Configure the DMA TX */

  if (HAL_DMA_Init(&hLPDMA2_CH0, HAL_LPDMA2_CH0) != HAL_OK)
  {
    return NULL;
  }
  ...
  xfer_cfg_usart2_tx_dma.request         = HAL_LPDMA2_REQUEST_USART2_TX;
  ...
  if (HAL_DMA_SetConfigPeriphDirectXfer(&hLPDMA2_CH0, &xfer_cfg_usart2_tx_dma) != HAL_OK)
  {
    return NULL;
  }

  /* Enable the interruption for LPDMA2_CH0 */
  HAL_CORTEX_NVIC_SetPriority(LPDMA2_CH0_IRQn, HAL_CORTEX_NVIC_PREEMP_PRIORITY_1, HAL_CORTEX_NVIC_SUB_PRIORITY_0);
  HAL_CORTEX_NVIC_EnableIRQ(LPDMA2_CH0_IRQn);

  /* Link the Transmit DMA handle to the UART handle */
  if (HAL_UART_SetTxDMA(&hUSART2, &hLPDMA2_CH0) != HAL_OK)
  {
    return NULL;
  }
...
/******************************************************************************/
/*                      LPDMA2 channel0 global interrupt                      */
/******************************************************************************/
void LPDMA2_CH0_IRQHandler(void)
{
  HAL_DMA_IRQHandler(&hLPDMA2_CH0);
}
